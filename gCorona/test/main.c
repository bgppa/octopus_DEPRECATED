...
/* At the same time, let's try optimizing the least square distance */
#include "optm.h"
...
double least_square (int d, const double* u)
{
	...
	double res = nrm2dist (Gu, glob_y, glob_dCod);
	free(Gu);
	return res;
}
...
//	x[1] = floor(rndmGaussian(40000, 5000., NULL));
	...
	double op_res = gradDesc (st, 2, least_square, lam, 900000, 0);
	printf("Minimum of value %f in the point\t", op_res);
	printVec(st, 2);
	printf("RES error: %f\n", op_res * 100. / nrm2(glob_y, glob_dCod));
	printf("MAP error: %f\n", nrm2dist(st, x, 2) * 100. / nrm2(x, 2));
	getchar();
	st[0] = 0.5, st[1] = 40;
	op_res = nwtnZero (st, 2, least_square, 2000, 3.0);
	printf("Minimum of value %f in the point", op_res);
	printVec(st, 2);
	getchar();
#endif

...
int randomInput (double *x)
{
	...
	x[1] = floor(rndmGaussian(100, 20, NULL));
	...
	/* Generate a random input suitable for the Corona model */
...
int main() {
	...
	int n_iterations = 200;/* Depth of every chain generating 1 sample */
...
setbuf(stdout, NULL);
		...
		start_pt[0] = fabs(rndmGaussian(0, 1., NULL));
       		start_pt[1] = floor(rndmGaussian(100, 20, NULL));
		printf("###### TEST %d of %d ######\n", a + 1, tot_test);
		randomInput(true_x);
//getchar();
        ...
        prll_uPcnSampler(potU, glob_dDom, start_pt, n_samples, n_iterations,
                        raw_samples, beta, seed_r, positive);
	#else	/* Ordinary pcn */
        uPcnSampler(potU, glob_dDom, start_pt,
                        n_samples, n_iterations, raw_samples, beta, positive);
	#endif

...
/* This library mostrly contains elementary linear algebra solvers.
 * in case of reasonabily general good results, I plan to replace
 * all this part with the more efficient LAPACK library. */
...
void touptri(double* A, double* y, int n){
	...
	for(m=0; m<n-1; ++m){
		for(j=m+1; j<n; ++j){
			...
			pivot = A[j*n+m];
			for(k=m; k<n; ++k){
				A[n*j+k] -= A[m*n+k] * (pivot/A[m*n+m]);
			}
			y[j] -= pivot*y[m]/A[m*n+m];
		}
	}
}
...
double det(double* A, int n){
	...
	int m, k, j;
	double pivot;
	/* Repeat the septs done via the gauss elimination... */
	for(m=0; m<n-1; ++m){
		for(j=m+1; j<n; ++j){
			pivot = A[j*n+m];
			for(k=m; k<n; ++k){
	...
	/*Now the matrix is ordered in a way to allow an easy det computation*/
	for (m=0, pivot=1.0; m<n; ++m)
		pivot *= A[n*m+m]; 
	return pivot;
}
...
void invgse(const double* A, double* B, int n){
	...
	for(int i=0; i<n; ++i){
		...
		ei(e_tmp, n, i+1);
		//printf("Solving the system for e_%d\n", i+1);
		//printVec(e_tmp, n);
		solgse(copyofA, e_tmp, n);
		/* ^---< there a KEY STEP. Can indeed use a faster method.
				 * can play in comparing them. */
		/* The result is now stored in e_tmp */
		/* so, e_tmp is the i-th column of the inverse matrix:
		 * let's copy it in B */
		//printf("DEBUG: %d-colonna: \n", i);
		//printVec(e_tmp, n);
		for(q=0; q<n; ++q){
			B[n*q + i] = e_tmp[q];
		}
...
int sor(const double* A, double* b, double* x, int d, double omega, \
						 double eps, int nmax){
	assert(A != NULL);
	assert(b != NULL);
	assert(x != NULL);
...
int jacobi(const double* A, double* b, double* x_next, int d, \
					double eps, int nmax){
	assert(A != NULL);
	assert(b != NULL);
	assert(x_next != NULL);
	...
	while( err>eps && ndone<nmax ){
		...
		/* x_next is now the candidate. Compute the error. */
		/* (computes the error of the operation Ax_next - b)*/

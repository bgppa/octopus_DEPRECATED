/* Code for implementing some basic linear algebra operations.
 * The author is aware about how it would have been much faster just
 * to use the well-kown Lapack libraries, but he wanted to write them again
 * as a form of exercise, and in order to guarantee a complete 100% control
 * of the code. Note that the LAPACK support can be added later, if required */

/* General rules to keep in mind:
 * 1) the comments are available in the headers, too
 * 2) Every function ASSUME to receive a pointer wich
 	have been properly allocated;
 * 3) NULL pointers, negative/zero dimensions are generally not allowed
	 (they are handled via the assert() C function) */
...
/* IMPORTANT: the use of all these functions is commented again in the
 * header BASIC.h */
...
void gemv(const double* A, double* y, const double* x, double alpha, \
	...
	/* A is a matrix of dimension dimy * dimx */
	double tmp=0;
	int j;
